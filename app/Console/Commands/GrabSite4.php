...
class GrabSite4 extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'grab:site4';
    ...
    public function handle()
    {
        ...
        $scrapedlinks = ScrapedLink::where('website_id', $id)->where('id','>','6991')->cursor();
        
        foreach ($scrapedlinks as $item) {
            $product=Product::where('links_id',$item->id)->count();
            if ($product) {
                $this->info('item exits :'.$item->id);
            }else{
            try {
