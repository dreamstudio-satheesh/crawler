...
class GrabSite3 extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'grab:site3';
            ...
            try {
                DB::transaction(function () use ($item, $website) {
                    ...
                    if ($node->count() > 0) {
                        ...
                        $priceText = trim($node->text()); // Get the text within the <h2> element
                        // Remove the del tag and its content
                        $priceText = preg_replace('/<del>.*<\/del>/', '', $priceText);
                        // Extract the price portion (remove non-numeric characters)
                        $priceText = preg_replace('/[^\d.]/', '', $priceText);
                        // Format the price with the currency symbol
                        $data['price'] = '₹' . number_format((float) $priceText, 2, '.', ',');
                    } else {
